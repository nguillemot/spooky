...
// "Spooky" modifications: Use CHECK_HR instead of ThrowIfFailed
...
OrbitCamera::OrbitCamera()
{
    ...
    CHECK_HR(CreateInteractionContext(&mInteractionContext));
    CHECK_HR(SetPropertyInteractionContext(mInteractionContext, INTERACTION_CONTEXT_PROPERTY_FILTER_POINTERS, TRUE));

    {
        ...
        CHECK_HR(SetInteractionConfigurationInteractionContext(mInteractionContext, ARRAYSIZE(config), config));
    }

    CHECK_HR(RegisterOutputCallbackInteractionContext(mInteractionContext, OrbitCamera::StaticInteractionOutputCallback, this));
}
...
void OrbitCamera::Projection(float fov, float aspect)
{
    ...
    mProjection = XMMatrixPerspectiveFovLH(fovY, aspect, 1.f, 1000.0f);
    UpdateData();
}
